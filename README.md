...
\^|[\s] — начало строки ИЛИ любой пробельный символ (пробел, табуляция, перевод строки и т.д.). То есть перед идентификатором не должно быть буквы/цифры, только начало строки или пробел.  
...
\(?=[\s]|$) — положительный просмотр вперёд: после идентификатора должен быть пробел или конец строки, но эти символы не включаются в результат.  

Пример совпадений:  
foo, Bar, \$var, \_temp  

Не совпадёт:  
123abc, foo\@bar, abc.def  

\(?<!\d)((?:\+7|8)[\s\-]*(?:\(?\d{3}\)?[\s\-]*)?\d{3}[\s\-]*\d{2}[\s\-]*\d{2})(?!\d)  
Пояснение по частям:  
(?<!\d) — отрицательный просмотр назад: перед номером не должно быть цифры (чтобы не схватить часть длинного числа).  
...
(?!\d) — отрицательный просмотр вперёд: после номера не должно быть цифры.  

Примеры совпадений:  
+7 123 456 78 90, 8(123)456-78-90, +7-123-456-78-90  

Не совпадёт:  
1234567 (нет +7/8), +71234567890 (без разделителей — всё равно найдёт, но если рядом цифра, то не найдёт).  
